...
export default function Pong() {
  ...
  useEffect(() => {
    tickRef.current = (ts: number) => {
      ...
      const leftX = 4 + PAD_W;
      if (s.ball.x - BALL_SIZE / 2 <= leftX && s.ball.x > 4 && s.ball.y >= s.player.y && s.ball.y <= s.player.y + PAD_H) {
      ...
      if (s.ball.x + BALL_SIZE / 2 >= rightX && s.ball.x < W - 4 && s.ball.y >= s.cpu.y && s.ball.y <= s.cpu.y + PAD_H) {
  ...
  const moveUp = (active: boolean) => { if (active) keysRef.current.add('ArrowUp'); else keysRef.current.delete('ArrowUp'); };
  const moveDown = (active: boolean) => { if (active) keysRef.current.add('ArrowDown'); else keysRef.current.delete('ArrowDown'); };
  ...
  return (
    <div
      ...
      onKeyUp={handleKeyUp}
      onPointerDown={() => { if (phaseRef.current === 'idle' || phaseRef.current === 'over') startGame(); }}
    ...
    >
      <canvas ref={canvasRef} width={W} height={H} style={{ border: '1px solid #000', imageRendering: 'pixelated', display: 'block' }} aria-label="Pong game" />
      {!isMobile ? (
        <div style={{ marginTop: 4, fontFamily: 'var(--font-chicago)', fontSize: 8, color: '#555' }}>W / S or Arrow keys to move</div>
      ) : (
        <div style={{ marginTop: 12, display: 'flex', gap: 20 }}>
          <button style={btnStyle} onPointerDown={(e) => { e.stopPropagation(); moveUp(true); }} onPointerUp={() => moveUp(false)} onPointerLeave={() => moveUp(false)}>UP</button>
          <button style={btnStyle} onPointerDown={(e) => { e.stopPropagation(); moveDown(true); }} onPointerUp={() => moveDown(false)} onPointerLeave={() => moveDown(false)}>DOWN</button>
        </div>
      )}
    </div>
  );
